    ...
    <div className="status-pipeline">
      <div className="status-pipeline__track">
        {PIPELINE_STEPS.map((step, idx) => {
          ...
          const failed  = currentStatus === 5
          const color   = failed && active ? STATUS_COLORS[5] : STATUS_COLORS[step]
          ...
          return (
            <div key={step} className="status-pipeline__step-wrap">
              {idx > 0 && (
                <div
                  className={`status-pipeline__connector ${done ? 'status-pipeline__connector--done' : ''}`}
                  style={done ? { background: `linear-gradient(90deg, ${STATUS_COLORS[step - 1]}, ${STATUS_COLORS[step]})` } : {}}
                />
              )}
              <div className="status-pipeline__step">
                <motion.div
                  ...
                  style={{
                    ...
                    boxShadow: active ? `0 0 16px ${color}55` : done ? `0 0 8px ${color}33` : 'none',
                  }}
                ...
                >
                  {done ? (
                    ...
                    <motion.div
                      className="status-pipeline__pulse"
                ...
                <div className="status-pipeline__label">
                  <span
                    className="status-pipeline__name"
                    style={{ color: done || active ? color : 'var(--text-muted)' }}
                  >
                    {STATUS_LABELS[step]}
                  </span>
                  {info?.timestamp && (
                    <span className="status-pipeline__time">
                      {formatTimestamp(info.timestamp)}
                    </span>
                  )}
                  {info?.txHash && (
                    
                      className="status-pipeline__tx"
                      href={explorerTx(info.txHash)}
                      target="_blank"
                      rel="noreferrer"
                    >
                      {truncateHash(info.txHash)}↗
                    </a>
                  )}
                </div>
              </div>
      ...
      {selectedExecutor && currentStatus >= 2 && (
        <motion.div
          className="status-pipeline__executor"
      ...
      {currentStatus === 5 && (
        <motion.div
          className="status-pipeline__failed-badge"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        >
          ✕ PAYMENT FAILED
