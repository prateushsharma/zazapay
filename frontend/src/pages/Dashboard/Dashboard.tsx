    ...
    <div className="dashboard">
      ...
      <section className="dashboard__hero">
        ...
        <div className="dashboard__hero-copy">
          <motion.h1 className="dashboard__hero-title" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            One Intent.<br />
            <span className="dashboard__hero-accent">Zero Intervention.</span>
          </motion.h1>
          <motion.p className="dashboard__hero-sub" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.15 }}>
            dApps submit a payment intent once. Somnia Agents plan, negotiate, execute, verify, and receipt entirely on-chain.
          </motion.p>
          {!intentId && (
            <motion.button className="dashboard__hero-cta" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} onClick={() => setShowForm(true)}>
      ...
      <div className="dashboard__grid">
        <div className="dashboard__col-main">
          ...
          <AnimatePresence>
            {intentId && lastTxHash && (
              <motion.div className="dashboard__submitted-banner" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }}>
                ...
                <div className="dashboard__submitted-right">
                  <a className="dashboard__submitted-tx" href={explorerTx(lastTxHash)} target="_blank" rel="noreferrer">{truncateHash(lastTxHash)}</a>
                  <button className="dashboard__submitted-new" onClick={() => { setIntentId(null); setLastTxHash(null); setShowForm(true) }}>
                    New Intent
                  </button>
                </div>
              </motion.div>
          ...
          <AnimatePresence>
            {intent && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
                <StatusPipeline currentStatus={intent.status} selectedExecutor={intent.selectedExecutor} />
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <AnimatePresence>
            {intent && intent.status >= 3 && payoutData.length > 0 && (
              <motion.div className="dashboard__payout" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <ResponsiveContainer width="100%" height={140}>
                  <BarChart data={payoutData} layout="vertical" margin={{ left: 16, right: 24 }}>
                    <XAxis type="number" domain={[0, 100]} tickFormatter={v => `${v}%`} tick={{ fill: 'var(--text-muted)', fontSize: 11, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} />
                    <YAxis type="category" dataKey="name" width={64} tick={{ fill: 'var(--text-secondary)', fontSize: 11, fontFamily: 'JetBrains Mono' }} axisLine={false} tickLine={false} />
                    <Tooltip contentStyle={{ background: '#0d0d14', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, fontFamily: 'JetBrains Mono, monospace', fontSize: '0.72rem', color: '#f0f0f0' }} formatter={(v: number) => [`${v}%`]} />
                    <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                      {payoutData.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </motion.div>
          ...
          {intentId && <TraceTimeline events={events} loading={traceLoading} />}
        </div>

        <div className="dashboard__col-side">
          <AgentPanel agents={agents} loading={agentsLoading} selectedExecutor={intent?.selectedExecutor} failedExecutor={failedExecutor} />
        </div>
      </div>
    </div>
  )
